...
namespace Sandcastle.Tools.BuildComponents.Commands
{
    ...
    public class SqlIndexedCache : InMemoryIndexedCache
    {
        ...
        private readonly List<SqlDictionary<string>> sqlCaches;
        ...
        /// <inheritdoc />
        public override int Count => base.Count + sqlCaches.Sum(c => c.Count);
        ...
        private SqlDictionary<string> CreateCache(XPathNavigator configuration)
        {
            ...
            if(cache == null)
            {
                if(!Boolean.TryParse(parent.GetAttribute("cacheProject", String.Empty), out bool cacheProject))
                    cacheProject = false;
