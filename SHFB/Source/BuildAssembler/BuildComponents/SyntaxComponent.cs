...
<containerElement name=""codeSnippetGroup"" addNoExampleTabs=""true"" includeOnSingleSnippets=""false""
    ...
    private void TransformComponent_TopicTransforming(object sender, EventArgs e)
    {
        ...
        foreach(XPathNavigator navCode in codeList)
        {
            ...
            // replaced later with actual code by the Code Block Component.  If there are no attributes and
            // it's not in a code container, assume it is a code inline element and skip it.
            if(code.ParentNode == null || code.ParentNode.LocalName == "code" || (code.Attributes.Count == 0 &&
              !codeContainers.Contains(code.ParentNode.LocalName)))
            {
                continue;
            }
