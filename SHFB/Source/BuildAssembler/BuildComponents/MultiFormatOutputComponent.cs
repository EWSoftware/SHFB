        ...
        public override void Initialize(XPathNavigator configuration)
        {
            ...
            foreach(XPathNavigator set in outputSets)
            {
                ...
                if(String.IsNullOrWhiteSpace(format))
                    throw new ArgumentException("You must specify a string value for the " +
                        "<helpOutput> 'format' attribute.", nameof(configuration));
        ...
        public override void Apply(XmlDocument document, string key)
        {
            ...
            if(formatComponents.Count == 1)
            {
                ...
                foreach(var componentSet in formatComponents.Values)
                {
                    ...
                    var clone = new XmlDocument { PreserveWhitespace = document.PreserveWhitespace };
