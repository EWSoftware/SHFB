...
namespace SandcastleBuilder.MSBuild
{
    ...
    public class BuildOpenXmlFile : Task
    {
        ...
        private static readonly Regex reBadChars = new Regex("[^_0-9A-Za-z]");
        private static readonly Regex reLineBreaks = new Regex("\r\n|\r|\n");
        ...
        private static readonly HashSet<string> ignoredParents = new HashSet<string> { "a", "align", "posOffset",
            "span", "t" };
        ...
        private void SaveRelationships()
        {
            ...
            this.Log.LogMessage(MessageImportance.High, "{0} external hyperlink relationship(s) defined",
                externalHyperlinks.Count);
            this.Log.LogMessage(MessageImportance.High, "{0} image relationship(s) defined", images.Count);
        }
        ...
        private void SaveNumberingStyles()
        {
            ...
            this.Log.LogMessage(MessageImportance.High, "{0} list numbering style override(s) defined",
            ...
            foreach(var element in elements.ToList())
            {
                ...
                if(localName != "p" && localName != "a" && localName != "span" && localName != "hyperlink")
                {
                    ...
                    do
                    {
                        ...
                        if(!(currentElement.NextNode is XElement nextNode))
                            break;
            ...
            foreach(var anchor in document.Descendants("a").ToList())
            {
                ...
                else
                {
                    ...
                    if(href.IndexOfAny(new[] { ':', '/', '\\' }) != -1)
                    {
                        ...
                        id = String.Format(CultureInfo.InvariantCulture, "elId{0:X}", href.ToUpperInvariant().GetHashCodeDeterministic());
        ...
        private void DetermineImageSize(string imageFilename, out long cx, out long cy)
        {
            ...
            if(!imageSizes.TryGetValue(imageFilename, out SizeF size))
            {
                ...
                if(!File.Exists(imageFilename))
                    this.Log.LogWarning("Unable to locate image file: {0}", imageFilename);
                else
        ...
        private void CompressHelpContent()
        {
            ...
            archiveName = Path.Combine(this.OutputFolder, this.HelpFilename) + ".docx";
            ...
            Log.LogMessage(MessageImportance.High, "Finished saving {0}\r\n" +
                ...
                archiveName, fileCount, uncompressedSize - archiveInfo.Length,
