...
public class MSBuildProject : IDisposable
{
    ...
    private static readonly Regex reInvalidAttribute = new(
        ...
        get
        {
            ...
            if(targetFrameworks == null)
            {
                if(properties.TryGetValue("TargetFrameworks", out ProjectProperty prop))
                    targetFrameworks = prop.EvaluatedValue.Split(separator, StringSplitOptions.RemoveEmptyEntries);
                else
                    targetFrameworks = [];
            }
    ...
    public string TargetFrameworkIdentifier
    {
        get
        {
            ...
            return NuGetFramework.Parse(this.TargetFramework).Framework;
        }
    }
        ...
        get
        {
            ...
            switch(this.TargetFrameworkIdentifier)
            {
                ...
                default:
                    ...
                    {
                        var tfi = NuGetFramework.Parse(this.TargetFramework);

                        if(tfi.Version.Build == 0)
                            versionValue = tfi.Version.ToString(2);
                        else
                            versionValue = tfi.Version.ToString(3);
                    }
                    break;
            }
    ...
    public MSBuildProject(string projectFile)
    {
        ...
        if(Path.GetExtension(projectFile).Equals(".vcproj", StringComparison.OrdinalIgnoreCase))
        {
      ...
      bool usesProjectSpecificOutput)
    {
        ...
        if(removeProjectWhenDisposed)
        {
            ...
            {
                List<string> values = [.. this.ProjectFile.ConditionedProperties[platformPropertyName]];

                if(values.IndexOf(platform) == -1 && values.IndexOf(BuildItemMetadata.DefaultPlatform) != -1)
