...
namespace SandcastleBuilder.MSBuild.BuildEngine
{
    ...
    public class MarkdownContentGenerator
    {
        ...
        private static readonly Regex reAddNewLinesBefore = new(@"(\w)(\<(p|div|h[1-6]|blockquote|pre|table|dl|" +
        ...
        private static readonly Regex reAddNewLinesAfter = new(@"(/(p|div|h[1-6]|blockquote|pre|table|dl|" +
            "ol|ul|address|script|noscript|form|fieldset|iframe|math|article|aside|canvas|details|dialog|" +
            @"figcaption|figure|footer|header|main|nav).*?\>)(\s*?\w)");
        private static readonly Regex reTrimNbsp = new(@"\s*&nbsp;\s*");
        private static readonly Regex reDecodeEntities = new("```.*?```", RegexOptions.Singleline);
        private static readonly MatchEvaluator meDecodeEntities = new(
        ...
        private string ApplyChanges(string key, XDocument topic)
        {
              ...
              String.IsNullOrWhiteSpace(s.Value)).ToList())
            {
                ...
                if(span.PreviousNode is XText sectionTitle)
                {
                    ...
                    string title = sectionTitle.Value.Split(carriageReturnLineFeed,
            ...
            if(topicTitle == null)
            {
                ...
                if(textBlock != null)
                {
                    string title = textBlock.Value.Split(carriageReturnLineFeed,
