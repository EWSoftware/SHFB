...
namespace SandcastleBuilder.MSBuild.BuildEngine
{
    public partial class BuildProcess
    {
        ...
        private void LoadPlugIns()
        {
            ...
            foreach(var dp in this.PresentationStyle.PlugInDependencies)
            {
            ...
            foreach(var kv in projectPlugIns)
            {
                ...
                try
                {
                    ...
                    if(this.PartialBuildType == PartialBuildType.None || plugIn.Metadata.RunsInPartialBuild)
                    {
                        ...
                        this.LoadedPlugIns.Add(kv.Key, instance);
                    }
                }
                catch(Exception ex)
                {
        ...
        private bool ExecutePlugIns(ExecutionBehaviors behavior)
        {
            ...
            executeList = [.. this.LoadedPlugIns.Values.Where(p => p.ExecutionPoints.RunsAt(step, behavior))];
