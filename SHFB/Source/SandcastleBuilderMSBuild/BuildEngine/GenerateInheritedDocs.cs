...
public class GenerateInheritedDocs
{
    ...
    private readonly string[] dupAttrs = ["cref", "href", "name", "vref", "xref"];
    ...
    private readonly string[] lastResortInheritableTypes =
    [
    ...
    /// </summary>
    public ReflectionFiles ReflectionFiles { get; } = new();
  ...
  <members>
    ...
    public bool Execute()
    {
        ...
        try
        {
            ...
            this.ReflectionFiles.AddReflectionFile(buildProcess.ReflectionInfoFilename);
    ...
    private void ScanCommentsFiles()
    {
        ...
        foreach(XPathNavigator file in commentsFiles)
        {
            foreach(XPathNavigator tag in file.Select("//inheritdoc"))
            {
                ...
                apiId = this.ReflectionFiles.SelectSingleNode("api[@id='" + name + "']");
        ...
        foreach(XPathNavigator api in this.ReflectionFiles.Select("api[proceduredata/@eii='true']/@id"))
        {
        ...
        foreach(XPathNavigator api in this.ReflectionFiles.Select(
        ...
        {
            // Look for the special extension block inheritdoc instances and add an inheritdoc element for the
            ...
            foreach(XmlNode inheritTag in n.SelectNodes("inheritdoc"))
            {
                ...
                {
                    ebIndex = cref.IndexOf('.', ebIndex);

                    if(ebIndex != -1)
                    {
                        cref = "T" + cref.Substring(1, ebIndex - 1);

                        string key = commentsCache.AllKeys.FirstOrDefault(
                            k => k.StartsWith(cref + ".", StringComparison.Ordinal));

                        if(key != null)
                        {
                            var parent = inheritTag.ParentNode;

                            while(parent.Name != "member")
                                parent = parent.ParentNode;

                            var typeInheritTag = inheritedDocs.CreateElement("inheritdoc");
                            
                            typeInheritTag.Attributes.Append(inheritedDocs.CreateAttribute("cref")).Value = key;
                            typeInheritTag.Attributes.Append(inheritedDocs.CreateAttribute("path")).Value =
                                "param|typeparam|returns|exception";

                            parent.AppendChild(typeInheritTag);
                        }
                    }
                }
            }
    ...
    private void InheritDocumentation(XmlNode member)
    {
        ...
        {
            StringBuilder sb = new("Circular reference detected: ");
        ...
        foreach(XmlNode inheritTag in member.SelectNodes("inheritdoc"))
        {
            ...
            commentsFound = false;
            sources.Clear();
            apiNode = this.ReflectionFiles.SelectSingleNode("api[@id='" + name + "']");
            ...
            else
            {
                ...
                if(namePosition != -1)
                {
                    ...
                    apiNode = this.ReflectionFiles.SelectSingleNode("api[@id='" + baseMember.Value + "']");
            ...
            foreach(XPathNavigator element in fromMember.Select(filter))
            {
                switch(element.Name)
                {
                    ...
                    default:
                        ...
                        {
                            // Ignore if there is a duplicate by attribute name and value
                            duplicate = null;
                            ...
                            foreach(string attrName in dupAttrs)
                            {
                                ...
                                if(!String.IsNullOrEmpty(attrValue))
                                {
                                    ...
                                    // Exception is a special case as there may be multiples for the name type
                                    // but with different content.
                                    if(duplicate != null && (element.Name != "exception" ||
                                      element.InnerXml == duplicate.InnerXml))
                                    {
                                        break;
                                    }
            ...
            else
            {
                if(inheritTag.ParentNode.Name != "member")
                {
                    ...
                    autoFilter = inheritTag.ParentNode.Attributes["cref"] ??
                        inheritTag.ParentNode.Attributes["name"];
