...
public class ContentLayoutEditorPane : SimpleEditorPane<ContentLayoutEditorFactory, ContentLayoutEditorControl>
{
    ...
    public ContentLayoutEditorPane()
    {
        ...
        editor.CommandBindings.Add(new CommandBinding(EditorCommands.ConvertToMarkdown,
...
#pragma warning restore VSTHRD010
    ...
    {
        e.CanExecute = this.UIControl.CurrentTopic?.TopicFile?.MarkdownFile is null;
    }
    ...
    private void cmdConvertToMarkdown_Executed(object sender, ExecutedRoutedEventArgs e)
    {
        ...
        switch(dlg)
        {
            case var d when d.SelectedTopicOnly:
                topics.Add(this.UIControl.CurrentTopic);
                break;

            case var d when d.SelectedTopicAndChildren:
                topics.Add(this.UIControl.CurrentTopic);
                topics.AddRange(this.UIControl.CurrentTopic.Subtopics.All());
                break;

            case var d when d.SelectedTopicAndSiblings:
                if(this.UIControl.CurrentTopic.Parent == null)
                    topics.AddRange(this.UIControl.Topics);
                else
                    topics.AddRange(this.UIControl.CurrentTopic.Parent);
                break;
        ...
        if(VsShellUtilities.ShowMessageBox(this, $"You are about to convert {topics.Count} MAML topic file(s) " +
          ...
          "project and content layout file.  The old MAML topics will be renamed with a '.aml_bak' extension.  " +
          "Do you want to continue?", Resources.PackageTitle, OLEMSGICON.OLEMSGICON_QUERY,
          OLEMSGBUTTON.OLEMSGBUTTON_YESNO, OLEMSGDEFBUTTON.OLEMSGDEFBUTTON_FIRST) == NativeMethods.IDNO)
        {
            return;
        }
        ...
        try
        {
            ...
            MamlToMarkdownTopicConverter converter = new(contentLayoutFile.Project.Language?.Name,
                contentLayoutFile.Project.ContentFiles(Sandcastle.Core.Project.BuildAction.ResourceItems).Select(
                    f => f.FullPath))
            {
        ...
        {
            Utility.ShowMessageBox(OLEMSGICON.OLEMSGICON_WARNING,
