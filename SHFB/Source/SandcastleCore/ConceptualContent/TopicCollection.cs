...
public class TopicCollection : BindingList<Topic>, ITableOfContents
{
    ...
    public void MatchProjectFilesToTopics()
    {
        ...
        {
            ext = Path.GetExtension(contentFile.Filename);

            if(ext.Equals(".aml", StringComparison.OrdinalIgnoreCase) ||
    ...
    private void SetTopic(TopicFile topicFile)
    {
        foreach(Topic t in this)
        {
              ...
              t.AlternateId == topicFile.AlternateId) || (t.TopicFile?.FullPath.Equals(topicFile.FullPath) ?? false))
            {
                ...
                // Markdown files store these properties for themselves
                if(t.TopicFile.MarkdownFile != null)
                {
                    t.Title = t.TopicFile.MarkdownFile.Title;
                    t.TocTitle = t.TopicFile.MarkdownFile.TocTitle;
                    t.LinkText = t.TopicFile.MarkdownFile.LinkText;
                    
                    t.Keywords.Clear();
                    
                    foreach(string keyword in t.TopicFile.MarkdownFile.Keywords)
                        t.Keywords.Add(new MSHelpKeyword(null, keyword));
                }
            }
    ...
    /// <remarks>Only actual conceptual content topic files are added.  They must have a ".aml" or ".md"
    /// extension and must be one of the valid <see cref="DocumentType">document types</see>.  Folders will be
    /// added as sub-topics recursively.  If a file with the same name as the folder exists, it will be
    /// associated with the container node.  If no such file exists, an empty container node is created.</remarks>
    public void AddTopicsFromFolder(string folder, string basePath, ISandcastleProject project)
    {
        ...
        // Add files
        foreach(string file in Directory.EnumerateFiles(folder, "*.md").Concat(
          Directory.EnumerateFiles(folder, "*.aml")))
        {
            try
            {
    ...
    public void GenerateTableOfContents(TocEntryCollection toc, bool includeInvisibleItems)
    {
        ...
        foreach(Topic t in this)
        {
            if(t.Visible || includeInvisibleItems)
            {
                ...
                entry.Id = t.Id;
                entry.AlternateId = t.AlternateId;
