...
public class ListElement : Element
{
    ...
    /// <inheritdoc />
    public ListElement(string name) : base(name, true)
    {
    }
    #endregion
    ...
    private void RenderMamlList(TopicTransformationCore transformation, XElement element)
    {
        ...
        if(transformation is not MarkdownConversionTransformation)
        {
            ...
            else
            {
                ...
                // Open XML always uses an unordered list element and applies a class attribute to let the Open
                // XML file generator know how to style the list.
        ...
        {
            // Lists can be complicated as they may contain more than simple paragraphs.  As long as each list
            // item only contains text or paragraphs with text and inline elements, we'll do our best to
            // convert it to Markdown.  If not, pass it through as is.
            List<XElement> listItems;

            if(this.Name == "list")
                listItems = [.. element.Elements(Ddue + "listItem")];
            else
            {
                listItems = [.. element.Elements(Ddue + "step").Select(s =>
                {
            ...
            if(listItems.All(li => li.Nodes().All(n => n is XText || n is XComment ||
              (n is XElement e && e.Name.LocalName == "para"))))
            {
                var el = transformation.CurrentElement;

                if(!Int32.TryParse(start, out int itemNumber))
                    itemNumber = 1;

                if(elementName == "ul" && style == "nobullet")
                    el.Add("{class=\"nobullet\"}");
