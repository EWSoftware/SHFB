...
public abstract class TopicTransformationCore
{
    ...
    /// not be parsed.
    /// </summary>
    /// <value>A tuple containing an enumerable list of the block tags and do not parse tags</value>
    /// <remarks>These are used by the Markdown to MAML converter to determine how to convert certain
    /// elements to MAML.</remarks>
    public (IEnumerable<string> BlockTags, IEnumerable<string> DoNotParseTags) HtmlBlockAndDoNotParseTags =>
        (new HashSet<string>(elementHandlers.Where(h => h.Value.IsBlockElement).Select(
            h => h.Key)), new HashSet<string>(elementHandlers.Where(h => h.Value.DoNotParse).Select(
            h => h.Key)));
