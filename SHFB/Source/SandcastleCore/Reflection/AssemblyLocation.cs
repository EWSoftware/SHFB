            ...
            if(!Directory.Exists(al.Path))
            {
                ...
                if(path.EndsWith(@"\ref", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    var latestVersion = Directory.EnumerateDirectories(path).Select(
                        p => new { Version = new SdkVersion(IOPath.GetFileName(p)), Path = p }).OrderByDescending(
                        p => p.Version.Version).ThenByDescending(p => p.Version.SupplementalVersionInfo).FirstOrDefault();
    ...
    public void DetermineAssemblyDetails(bool clearAndRefresh)
    {
        ...
        if(!String.IsNullOrWhiteSpace(this.Path) && Directory.Exists(this.Path))
        {
            ...
            foreach(string sdkFolder in Directory.EnumerateDirectories(this.Path))
            {
                ...
                var version = Directory.EnumerateDirectories(sdkFolder).Select(
                    p => new { Version = new SdkVersion(IOPath.GetFileName(p)), Path = p }).OrderByDescending(
                    p => p.Version.Version).ThenByDescending(p => p.Version.SupplementalVersionInfo).FirstOrDefault();

                if(version != null)
                {
                    assemblyName = IOPath.Combine(version.Path, "ref", "dotnet",
                ...
                version = Directory.EnumerateDirectories(sdkFolder).Select(
                    p => new { Version = new SdkVersion(IOPath.GetFileName(p)), Path = p }).OrderByDescending(
                    p => p.Version.Version).ThenByDescending(p => p.Version.SupplementalVersionInfo).FirstOrDefault();

                if(version != null)
                {
                    assemblyName = IOPath.Combine(version.Path, sdkFolder.Substring(
