    ...
    public class DividerLabel : Label
	{
		...
		protected override void OnPaint(PaintEventArgs e)
		{
            ...
            using(StringFormat sf = new StringFormat())
            {
                ...
                using(Pen pb = new Pen(ControlPaint.Dark(this.BackColor), SystemInformation.BorderSize.Height))
                {
                    if(this.FlatStyle == FlatStyle.Flat)
                        g.DrawLine(pb, pt, pt2);
                    else
                    {
                        using(Pen pf = new Pen(ControlPaint.LightLight(this.BackColor), SystemInformation.BorderSize.Height))
                        {
                            g.DrawLine(pb, pt, pt2);
