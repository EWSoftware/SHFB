...
// Help File Builder features that need it.
//
...
namespace SandcastleBuilder.Gui.Gac
{
    ...
    internal enum ASM_CMP_FLAGS
    {
        ...
        CULTURE          = 0x040,
        CUSTOM           = 0x080,
        ALL = NAME | MAJOR_VERSION | MINOR_VERSION | REVISION_NUMBER | BUILD_NUMBER | PUBLIC_KEY_TOKEN |
            CULTURE | CUSTOM,
        DEFAULT          = 0x100
    }
    ...
    internal static class NativeMethods
    {
        ...
        /// </summary>
		/// <param name="ppEnum">A reference that will end up containing the IAssemblyEnum interface.</param>
		/// <param name="pUnkReserved">Reserved.  Must always be null.</param>
		/// <param name="pName">An assembly name that is used to filter the enumeration.  This can be null to
        /// enumerate all assemblies in the GAC.</param>
		/// <param name="dwFlags">Specify one (and only one) ASM_CACHE_FLAGS value.</param>
        ...
        internal static extern void CreateAssemblyEnum(out IAssemblyEnum ppEnum,
            IntPtr pUnkReserved, IAssemblyName pName, ASM_CACHE_FLAGS dwFlags, IntPtr pvReserved);
    }
}
