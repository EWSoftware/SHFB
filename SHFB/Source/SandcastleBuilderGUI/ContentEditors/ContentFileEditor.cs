...
namespace SandcastleBuilder.Gui.ContentEditors
{
    ...
    public class ContentFileEditor : ICloneable
    {
        ...
        [Category("Definition"), RefreshProperties(RefreshProperties.All),
         Description("The filename extensions for which this content file editor will be used.  Separate " +
        ...
        /// </summary>
        /// <remarks>The following macros can be used to substitute item and path values into the argument string.
        /// 
        /// <list type="table">
        ///    <listheader>
        ...
        ///       <description>The full path to the content item to be edited</description>
        ///    </item>
        ///    <item>
        ///       <term>$ProjectFile</term>
        ///       <description>The fully qualified path and filename of the current project.</description>
        ///    </item>
        ///    <item>
        ///       <term>$ProjectFolder</term>
        ///       <description>The fully qualified path to the current project.</description>
        ///    </item>
        /// </list>
        /// 
        /// <para>The macro names are case-insensitive.  Enclose arguments in double quotes if they may contain
        /// spaces.</para></remarks>
        [Category("Editor"), Description("The parameters to pass to the application.  $ContentItem = The " +
            "content item to edit, $ProjectFile = Full project path and filename, $ProjectFolder = Full " +
        ...
        /// application.
        /// </summary>
        [Category("Editor"), Description("The path to the additional content editor application."),
        ...
        public FilePath ApplicationPath
        {
            ...
            set
            {
                if(value == null || value.Path.Length == 0)
                    applicationPath = new FilePath(@"%SystemRoot%\System32\notepad.exe", null);
                else
        ...
        [Category("Editor"), Description("The startup folder used when launching the content file editor.  " +
          "If blank, the current project's path is used."),
          Editor(typeof(FolderPathObjectEditor), typeof(UITypeEditor)),
          FolderDialog("Select the startup path when launching the editor", Environment.SpecialFolder.MyDocuments),
          XmlIgnore]
        ...
        public string EditorDescription => String.Format(CultureInfo.CurrentCulture, "{0} ({1})", description,
            extensions);

        /// <summary>
        /// This property is used to serialize the application path
        /// </summary>
        /// <remarks><see cref="FilePath" /> is not serializable because it does not have a parameterless
        /// constructor.  This stands in for the <see cref="ApplicationPath" /> property when this class is
        /// serialized.</remarks>
        [Browsable(false)]
        ...
        /// </summary>
        /// <remarks><see cref="FolderPath" /> is not serializable because it does not have a parameterless
        /// constructor.  This stands in for the <see cref="StartupFolder" /> property when this class is
        /// serialized.</remarks>
        [Browsable(false)]
        ...
        public object Clone()
        {
            return new ContentFileEditor
            {
                Description = description,
                Arguments = arguments,
                Extensions = extensions,
                ApplicationPath = new FilePath(applicationPath.PersistablePath, null),
                StartupFolder = new FolderPath(startupFolder.PersistablePath, null)
            };
        }
        #endregion
        ...
        /// <returns>True if this is the editor for the given file extension or false if not.</returns>
        ...
        /// <returns>A string containing the actual arguments to pass to the editor application.</returns>
        public string ActualArguments(string contentItem, string projectFile)
        {
            ...
            actualArgs = reProjectFolder.Replace(actualArgs, Path.GetDirectoryName(projectFile));
