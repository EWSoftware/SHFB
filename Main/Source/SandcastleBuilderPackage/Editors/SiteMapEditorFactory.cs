...
namespace SandcastleBuilder.Package.Editors
{
    ...
    public sealed class SiteMapEditorFactory : SimpleEditorFactory<SiteMapEditorPane>
    {
        ...
        /// <inheritdoc />
        /// <remarks>The site map editor factory overrides this to ensure that a SHFB project is
        /// the active project before allowing the editor to open.  Site map files are tightly coupled
            ...
            out IntPtr ppunkDocData, out string pbstrEditorCaption, out Guid pguidCmdUI, out int pgrfCDW)
        {
            if(SandcastleBuilderPackage.CurrentSandcastleProject == null)
            {
                ...
                Utility.ShowMessageBox(OLEMSGICON.OLEMSGICON_INFO, "Site map files must be associated with " +
                    "and opened from a Sandcastle Help File Builder project.  Please create a project or " +
